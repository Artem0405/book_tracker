...
void main() async {
  ...
  // Создаем экземпляры наших ключевых сервисов, репозиториев и BLoC'ов
  final firebaseAuth = FirebaseAuth.instance;
  final bookRepository = BookRepository(firebaseAuth: firebaseAuth);
  final authBloc = AuthBloc(firebaseAuth: firebaseAuth);

  runApp(
    // Используем MultiRepositoryProvider для удобной регистрации нескольких зависимостей.
    // Это делает код более чистым, чем вложенные RepositoryProvider.
    MultiRepositoryProvider(
      providers: [
        // Предоставляем FirebaseAuth всему приложению
        RepositoryProvider.value(value: firebaseAuth),
        // Предоставляем BookRepository всему приложению
